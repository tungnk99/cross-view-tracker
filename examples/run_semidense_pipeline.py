"""
Example: Semi-Dense Pipeline - LoFTR + Homography

This example demonstrates the semi-dense pipeline with learned matching:
- Matcher: LoFTR (detector-free semi-dense matcher)
- Mapper: Homography with RANSAC

LoFTR/Efficient LoFTR is particularly effective for:
- Semi-dense local feature matching with high accuracy
- Cross-view matching with appearance changes
- Low-texture regions
- Large viewpoint changes

Reference: "LoFTR: Detector-Free Local Feature Matching with Transformers"
           "Efficient LoFTR: Semi-Dense Local Feature Matching with Sparse-Like Speed"

Usage:
    python examples/run_semidense_pipeline.py
"""
...
def visualize_results(source_img, target_img, result, roi_center=None):
    ...
    # Save figure
    output_path = os.path.join(project_root, 'examples', 'output_semidense.png')
...
def main():
    print("="*80)
    print("SEMI-DENSE PIPELINE EXAMPLE - LoFTR (Semi-Dense Matcher) + Homography")
    print("="*80)
    ...
    # Create pipeline
    config_path = os.path.join(project_root, 'configs', 'pipeline_semidense.yaml')
    print(f"\n📋 Loading config from: {os.path.basename(config_path)}")
    
    # Initialize pipeline with CPU (change to 'cuda' if you have GPU)
    device = 'cpu'
    print(f"\n⚠️  Note: Using device='{device}'")
    print(f"   LoFTR is a deep learning model and runs much faster on GPU.")
    print(f"   Change device to 'cuda' if you have a GPU available.")
    ...
    if 'debug_info' in result:
        ...
        print(f"   Avg Match Confidence: {debug.get('avg_match_confidence', 'N/A'):.3f}")
        ...
        if 'match_confidence' in debug and 'mapper_confidence' in debug:
            print(f"   Match Confidence: {debug['match_confidence']:.3f}")
            print(f"   Mapper Confidence: {debug['mapper_confidence']:.3f}")
